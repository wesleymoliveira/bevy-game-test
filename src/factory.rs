...
use crate::{Army, Bot, Textures, Thrust, Turret, Ufo};

pub struct Factory<'a, 'b: 'a, 'c: 'a> {
    pub commands: &'a mut Commands<'a, 'b>,
    pub textures: &'c Textures,
}

impl<'a, 'b, 'c, 'd> Factory<'a, 'b, 'c> {
    pub fn new(commands: &'a mut Commands<'a, 'b>, textures: &'c Textures) -> Self {
        Self { commands, textures }
    }

    pub fn spawn_bot(&mut self, x: f32, y: f32, parent: Entity) -> Entity {
        let tank = self.spawn_tank(x, y, parent);
        self.commands.entity(tank).insert(Bot::default()).id()
    }

    pub fn spawn_player(&mut self, x: f32, y: f32, parent: Entity) -> Entity {
        let tank = self.spawn_tank(x, y, parent);
        self.commands.entity(tank).insert(Ufo::default()).id()
    }
